...
unsigned __stdcall UDPsnd(void *UDPsnd)
{
	...
	sercer.sin_addr.S_un.S_addr = inet_addr("239.114.214.100");
	...
	while(FlagUDP)
	{
		...
		)
		{
			...
			{
				memcpy((unsigned char *)buf_tx,sizeof(double)*(4*4));
				len = sendto(sock_snd,str_tx,sizeof(double)*(4*4),0,(struct sockaddr *)&server,sizeof(server));
...
void savedata(void)
{
	...
	mtfile = fopen("logging_host.mat","w");
	for(i=0;i<cnt;i++)
	{
		for(j=0;j<cols;j++)
		{
